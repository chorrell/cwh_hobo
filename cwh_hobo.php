...
/**
 * cwh_hobo - Random Hobo Name Generator
 *
 * A Textpattern plugin that displays a random hobo name from
 * John Hodgeman's list of 700 hobo names from his book
 * "The Areas of my Expertise"
 *
 * @author  Christopher Horrell
 * @link    https://horrell.ca/
 * @license GPL-2.0
 * @version 1.0.0
 */
...
$plugin['author_uri'] = 'https://horrell.ca/';
$plugin['description'] = 'Generates a random hobo name from John Hodgeman\'s book "The Areas of my Expertise".';

// Plugin load order:
// The default value of 5 would fit most plugins
$plugin['order'] = 5;
...
// Plugin 'flags' signal the presence of optional capabilities
// Not needed for this simple plugin
$plugin['flags'] = 0;

// Load the template compiler if running from command line
...
<p>This plugin will display a random hobo name from John Hodgeman's list of 700 hobo names taken from his book <a href="https://www.amazon.ca/Areas-Expertise-Instructive-Annotation-2006-09-05/dp/B0163DW6CW/">The Areas of my Expertise</a> (more information <a href="https://en.wikipedia.org/wiki/The_Areas_of_My_Expertise">here</a>).</p>

<h2>Usage</h2>

<p>To use it, simply insert the following tag wherever you want a random hobo name to appear:</p>

<pre>&lt;txp:cwh_hobo /&gt;</pre>

<p>This will produce a hobo name and designated number, like so:</p>

<pre>#171: Twink the Reading-Room Snoozer</pre>

<h2>Attributes</h2>

<p>cwh_hobo accepts two attributes:</p>

<ul>
<li><code>wraptag</code> - HTML tag to wrap the output (default: none)</li>
<li><code>class</code> - CSS class to apply to the wraptag (default: empty)</li>
</ul>

<h3>Example</h3>

<p>If you want to wrap the output in a paragraph tag with the class of "hobo", you would have:</p>

<pre>&lt;txp:cwh_hobo wraptag="p" class="hobo" /&gt;</pre>

<p>which would produce:</p>

<pre>&lt;p class="hobo"&gt;#171: Twink the Reading-Room Snoozer&lt;/p&gt;</pre>

<h2>Requirements</h2>

<ul>
<li>Textpattern CMS 4.7+ (tested with 4.8.x and 4.9.0)</li>
<li>PHP 8.0+ (tested with PHP 8.4 and 8.5)</li>
</ul>
...
/**
 * Main plugin function - generates and outputs a random hobo name
 *
 * @param  array $atts Tag attributes
 * @return string The formatted hobo name
 */
function cwh_hobo($atts)
{
    // Parse attributes with safe defaults
    $atts = lAtts([
        'class'   => '',
        'wraptag' => '',
    ], $atts);

    // Get a random hobo name
    $hobo_name = cwh_hobo_get_random();

    // Return with optional wrapper tag
    return $atts['wraptag']
        ? doTag($hobo_name, $atts['wraptag'], $atts['class'])
        : $hobo_name;
}

/**
 * Get a random hobo name from the list
 *
 * @return string A random hobo name with number
 */
function cwh_hobo_get_random()
{
    static $hobos = null;

    // Initialize the hobo list only once (lazy loading)
    if ($hobos === null) {
        $hobos = cwh_hobo_get_list();
    }

    // Return a random hobo name
    return $hobos[array_rand($hobos)];
}

/**
 * Get the complete list of hobo names
 *
 * @return array Array of hobo names
 */
function cwh_hobo_get_list()
{
    return [
